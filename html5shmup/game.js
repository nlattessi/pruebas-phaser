...
BasicGame.Game.prototype = {
  ...
  setupEnemies: function() {
    ...
    this.shooterPool.createMultiple(50, 'greenEnemy');
  ...
  spawnEnemies: function() {
    if (this.nextEnemyAt < this.time.now && this.enemyPool.countDead() > 0) {
      ...
      // Spawn at a random locationtop of the screen
      enemy.reset(
  ...
  processDelayedEffects: function() {
    ...
    if (this.bossAproaching && this.boss.y > 80) {
      this.bossAproaching = false;
  ...
  damageEnemy: function(enemy, damage) {
    ...
    if (enemy.alive) {
      ...
      // For full games, it would be better to set flags on the sprite themselves
      if (enemy.key === 'boss') {
        this.enemyPool.destroy();
        this.shooterPool.destroy();
        this.bossPool.destroy();
  ...
  fire: function() {
    ...
    if (this.weaponLevel === 0) {
      ...
      for (var i = 0; i < this.weaponLevel; i++) {
        ...
        bullet.reset(this.player.x + (10 + i * 6), this.player.y - 20);
        // the left bullets spread from -85 degrees to -45 degrees
        this.physics.arcade.velocityFromAngle(
